...
# Set this site as default if it's the first one
if [ ! -f "/etc/nginx/sites-enabled/default" ] && [ "$(ls -A /etc/nginx/sites-enabled/ | wc -l)" -eq 1 ]; then
    log_message "Setting this site as the default server..."
    sed -i 's/listen 80;/listen 80 default_server;/' "/etc/nginx/sites-available/$DOMAIN.conf"
    sed -i 's/listen \[::\]:80;/listen [::]:80 default_server;/' "/etc/nginx/sites-available/$DOMAIN.conf"
fi
